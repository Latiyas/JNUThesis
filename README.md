...
2. 调整了枚举列表的默认距离。
3. 修改了图、表标题字体大小。（感谢Zhaonan Zhou同学的帮助）
4. 增加了附录页，并使用命令hasappendix参数控制其的显示与隐藏。（感谢Bifeng Guo同学的帮助）
...
2024.04.06更新
1. 将论文修改成双面模式，设置不同的奇偶页页眉，并可通过isprint参数控制是否包含打印必须的空白页。（感谢Jianglong Cao和Yaoyao Hong同学的帮助）
2. 重新调整代码的顺序，方便查阅。
...
2. 增加了非上标的引用格式\norcite{}，修正了章节引用的一些问题。（感谢Jiahong Liu和Bifeng Guo同学的帮助）
...
2023.03.17更新
1. 修正了目录、章节的标题对齐问题。（感谢Fangzhen Zhao同学的帮助）
2. 修正了参考文献引用问题。

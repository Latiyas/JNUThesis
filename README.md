...
同时，由于笔者论文写作经验较浅，一开始接触的便是Overleaf，所以情感上更倾向于在线版的Overleaf。私以为Overleaf存在以下几点优势：a.简洁，高效，省去了配置环境的麻烦；b.可以多人在线，共同编辑同一个文档。因此萌发了将暨南大学博硕士学位论文模板迁移至Overleaf上的念头。(PS: 现在迁移到TexPage了)
...
本项目虽然侧重于在TexPage上进行编译测试，但同样能够在本地Latex上的编译通过。部分同学也反映，本地编译的结果是正常的。由于作者精力有限，暂时没有余力对本地编译的所有问题进行一一排查，请有问题的同学自行百度解决。本项目在TexPage网站上的共享链接为[https://www.texpage.com/share/ef0b935f449f4729af70b76fcca72ece](https://www.texpage.com/share/ef0b935f449f4729af70b76fcca72ece)。
（注：暨南大学经济学院参考文献使用著者-出版年编码形式，该需求在新的项目中得到了相应支持[https://www.texpage.com/share/1df8d6beb79f488d9a99e41a3f845766](https://www.texpage.com/share/1df8d6beb79f488d9a99e41a3f845766)。）

PS: 论文模板的第二章包含了作者总结的常用的LaTex代码。这源于作者的经验以及部分用户的反馈中薄弱的部分，强烈建议用户在快速阅读一遍后再进行论文的撰写。当用到这些功能时，欢迎您能回到这里寻找灵感。

PPS: 由于作者精力有限，对于同学们反映的一些小问题，一般会先在TexPage项目中进行快速更新。除非有较大的变动，否则将会在一到两个月对Github上的项目进行一次全面更新，敬请谅解。


## 运行环境
--------------------------------------------------------
1. 从分享链接复制项目：
(1) 在线注册TexPage账号，点击链接进入共享项目（安全起见，普通用户仅可查看项目）。
(2) 退出当前项目，可以在你的项目列表找到JNUThesis项目。点击该项目右侧的按钮，选择"复制项目"，这样你就获得了当前项目的备份。
2. 从github网站复制项目：
(1) 在线注册TexPage账号，依次点击"新建->上传项目"，将从github网站上下载的zip文件上传即可。
(2) 进入Project，右上角打开"设置"选项，将"Latex编译器"修改成"XeLaTeX"，将"主文件"设置为"JNUThesis.tex"，选择"JNUThesis.tex"进行编译即可。
(3) 目前项目在Tex Live 2025版本下运行。如果运行异常，建议检查"设置->TeX Live版本"的设置。
...
3. chapters目录(根据需要引用章节，引用方式为"\include{需要引用的章节名称}"):
    > + abstract.tex: 中英文摘要。
...
对于本项目的早期使用者，如果希望与本项目的版本保持同步，强烈建议您在提前备份好自己的项目文件后，重新下载本项目主目录下的文件（主要是JNUThesis.tex，configs/theme.tex和gbt7714-2015.bst，前两个控制着论文的排版格式，后者决定了参考文献的格式）去替换您项目中的对应文件。得益于内容与样式的分离，一般情况下，版本的更新都能成功。根据作者的经验，发生错误的大部分情况是，文件夹或者tex文件的名称不匹配；新增的函数引用或者自定义命令放在主文件中而不是configs/libs.tex中，请使用者自行检查上述问题。
...
--------------------------------------------------------
2026.02.05更新
1. 项目迁移到TexPage网站。（感谢Siwei Liu的帮助）
2. 微调了项目结构，并修正了一些小错误。
...
## 注意事项
--------------------------------------------------------
1. 由于线上编译速度较慢。因此，撰写文稿时可以仅引用当前编辑的章节方便快速查看。暂未使用的章节可以先行注释，当需要阅读全文时再全部引用。正文章节的引用一般是放在chaps.tex中。
